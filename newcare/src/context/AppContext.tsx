...
interface ContextData {
  ...
  selecionarAcessorioAvatar: (acessorioId: string) => Promise<void>;
...
export function AppProvider({ children }: { children: ReactNode }) {
      ...
      try {
        ...
        if (user) {
          setUsuario({
            ...
            avatarId: ["avatar-1", "avatar-2", "avatar-3", "avatar-4"].includes(user.avatarId ?? "")
              ? "avatar-mental"
              : user.avatarId,
            avatarAccessoryId: user.avatarAccessoryId ?? "sem-item",
            preferencias: {
              ...PREFERENCIAS_PADRAO,
              ...user.preferencias,
            },
  ...
  async function login(email: string, senha: string) {
    ...
    const user: Usuario = {
      ...
      avatarId: "avatar-mental",
      avatarAccessoryId: "sem-item",
      perfil: {
  ...
  async function concluirOnboarding(perfil: OnboardingPerfil) {
    ...
    const atualizado: Usuario = {
      ...
      onboardingCompleto: true,
      habitosConfirmados: true,
      avatarId: usuario.avatarId ?? "avatar-mental",
      avatarAccessoryId: usuario.avatarAccessoryId ?? "sem-item",
      perfil,
    };
  ...
  async function selecionarAcessorioAvatar(acessorioId: string) {
    if (!usuario) return;

    const usuarioAtualizado: Usuario = {
      ...usuario,
      avatarAccessoryId: acessorioId,
    };
